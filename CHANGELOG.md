# Changelog

All notable changes to this project will be documented in this file.
<!-- markdownlint-disable line-length no-bare-urls ul-style emphasis-style -->

## [1.5.8] - 2026-09-22



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.7...openflows-1.5.8



## [1.5.7] - 2026-09-22



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.6...openflows-1.5.7



## [1.5.6] - 2026-09-22



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.5...openflows-1.5.6



## [1.5.5] - 2026-09-22



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.4...openflows-1.5.5



## [1.5.4] - 2026-09-18



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.3...openflows-1.5.4



## [1.5.3] - 2026-09-17



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.2...openflows-1.5.3



## [1.5.2] - 2026-09-17

### Bug Fixes

- *(ci,tenant)* Address review — resolver, tenant repo persist, owner-scoped warn by @Christiantyemele



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.1...openflows-1.5.2



## [1.5.1] - 2026-09-17



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.5.0...openflows-1.5.1



## [1.5.0] - 2026-09-17

### Features

- *(debug)* Add workspace provider function with fallback for debug output by @Christiantyemele



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.4.0...openflows-1.5.0



## [1.4.0] - 2026-09-17

### Features

- [fc12160](
https://github.com/The-AgenticFlow/openflows/commit/fc121604ed30e2dce29739e2f84541e01062738e) *(debug)* Surface compiled binary version in debug output

  > Print the openflows version (from CARGO_PKG_VERSION) at the top of
  > debug_system output so operators can correlate a debug dump with the
  > exact build that produced it.



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.3.3...openflows-1.4.0



## [1.3.3] - 2026-09-17



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.3.2...openflows-1.3.3



## [1.3.2] - 2026-09-16



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.3.1...openflows-1.3.2



## [1.3.1] - 2026-09-16



**Full Changelog**: https://github.com/The-AgenticFlow/openflows/compare/openflows-1.3.0...openflows-1.3.1


