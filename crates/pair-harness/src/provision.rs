...
impl Provisioner {
    ...
    /// When AI Gateway is disabled, also includes ANTHROPIC_API_KEY for direct API authentication.
    async fn provision_coder_terraform_vars(
        ...
        if let Some(ref module) = forge_module {
            // When AI Gateway is enabled, we use the gateway-discovered model ID (e.g. "adorsys-coder")
            // for the Terraform anthropic_model variable. This ensures the container's environment
            // receives a valid gateway catalog ID that the Coder bridge will accept, overriding any
            // stale or corrupted Terraform state from previous runs.
            // If the gateway is not enabled, we fall back to the registry's model_backend.
            let anthropic_model = if module.ai_gateway_enabled() && gateway_model.is_some() {
                gateway_model.unwrap().to_string()
            } else {
                model_backend
                    .filter(|m| m.starts_with("anthropic/"))
                    .map(|m| m.strip_prefix("anthropic/").unwrap().to_string())
                    .unwrap_or_else(|| "claude-haiku-4-5-20251001".to_string())
            };

            let openai_model = if module.ai_gateway_enabled() && gateway_model.is_some() {
                gateway_model.unwrap().to_string()
            } else {
                model_backend
                    .filter(|m| m.starts_with("openai/"))
                    .map(|m| m.strip_prefix("openai/").unwrap().to_string())
...
"#,
                ...
                anthropic_model,
                anthropic_model, // Use same model for both for now; can be customized per backend
                openai_model
            );
