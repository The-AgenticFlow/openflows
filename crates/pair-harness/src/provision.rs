...
impl Provisioner {
    ...
    pub async fn provision_pair(
        ...
        // When using a custom SSE provider (CODEX_USE_SSE=true), skip MCP config
        // generation for Codex backend because MCP tools use type="mcp" in the
        // Responses API, which non-OpenAI providers reject with "unknown tool type".
        // For Claude backend, MCP servers work fine regardless.
        let is_codex_sse = cli_backend == CliBackend::Codex && crate::process::codex_use_sse();
        if !backend_config.mcp_config_rel.as_os_str().is_empty() && !is_codex_sse {
    ...
    fn generate_codex_config_toml(
        ...
        // When using a custom OpenAI-compatible SSE provider (CODEX_USE_SSE=true),
...
# DO NOT EDIT — changes will be overwritten on next provision
#
# MCP servers are DISABLED because CODEX_USE_SSE=true is set.
# Non-OpenAI providers reject MCP tool types (type="mcp") in the Responses API,
# causing "unknown tool type" 400 errors. GitHub operations are available via
