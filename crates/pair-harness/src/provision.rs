...
impl Provisioner {
    ...
    pub async fn provision_pair(
        ...
        // Skip for Codex: MCP servers are embedded in .codex/config.toml
        // (generated later in provision_backend_extras). Writing a separate
        // mcp.json to the config.toml path would overwrite it with JSON,
        // then get overwritten again by the TOML generator — wasted work.
        if !backend_config.mcp_config_rel.as_os_str().is_empty()
            && cli_backend != CliBackend::Codex
        {
        ...
        // Same skip for Codex — SENTINEL config is in shared/.codex/config.toml
        if !backend_config.mcp_config_rel.as_os_str().is_empty()
            && cli_backend != CliBackend::Codex
        {
            let mcp_gen = crate::mcp_config::McpConfigGenerator::new(github_token, redis_url);
        ...
        // 4b. Pre-cache MCP server npm packages so Codex doesn't time out
        //     waiting for npx -y to download them on first use.
        //     Only the GitHub MCP server needs pre-caching now — the
        //     filesystem MCP server has been removed because Codex's exec
        //     mode cancels MCP write_file calls.
        if !backend_config.mcp_config_rel.as_os_str().is_empty()
            && cli_backend != CliBackend::Codex
        {
            self.precache_mcp_servers()?;
        }
...
# NOTE: The filesystem MCP server is intentionally excluded because Codex's
# exec mode cancels MCP write_file calls ("user cancelled MCP tool call").
# The model then gets stuck retrying MCP writes instead of using native
# exec/apply_patch tools.  Without the filesystem MCP server, the model
# uses exec (shell commands: cat, ls, mkdir, heredoc/tee for writes) and
# apply_patch for file modifications — both of which work correctly under
# the workspace-write sandbox in exec mode.
...
"#,
    ...
    fn generate_codex_agent_toml_for_role(&self, target: &Path, agent_id: &str) -> Result<()> {
        ...
        // Resolve model from env vars with agent-specific override priority.
        // FORGE_MODEL takes precedence over FIREWORKS_MODEL/OPENAI_MODEL so
        // that a more capable model (e.g. gpt-4o) can be used for FORGE even
        // when the default is a cheaper model (e.g. gpt-4o-mini).
        // gpt-4o-mini lacks the tool-use sophistication needed for Codex's
        // exec/apply_patch/write_stdin interface and will get stuck in
        // infinite write_stdin retry loops when trying to create files.
        let model = match agent_id {
            "forge" => std::env::var("FORGE_MODEL")
                .ok()
                .filter(|m| !m.is_empty())
                .or_else(|| std::env::var("FIREWORKS_MODEL").ok().filter(|m| !m.is_empty()))
                .or_else(|| std::env::var("OPENAI_MODEL").ok().filter(|m| !m.is_empty()))
                .unwrap_or_else(|| "gpt-4o".to_string()),
            "sentinel" => std::env::var("SENTINEL_MODEL")
                .ok()
                .filter(|m| !m.is_empty())
                .or_else(|| std::env::var("FIREWORKS_MODEL").ok().filter(|m| !m.is_empty()))
                .or_else(|| std::env::var("OPENAI_MODEL").ok().filter(|m| !m.is_empty()))
                .unwrap_or_else(|| "gpt-4o".to_string()),
            _ => std::env::var("FIREWORKS_MODEL")
...
"#
    ...
    /// Pre-cache MCP server npm packages so Codex doesn't time out waiting
    /// for `npx -y` to download them on first use.
    ///
    /// Only the GitHub MCP server needs pre-caching. The filesystem MCP
    /// server has been removed from the Codex config because Codex's exec
    /// mode cancels MCP `write_file` calls ("user cancelled MCP tool call"),
    /// which causes the model to get stuck retrying MCP writes instead of
    /// using native `exec`/`apply_patch` tools.
    pub fn precache_mcp_servers(&self) -> Result<()> {
        let mcp_packages = [
            "@modelcontextprotocol/server-github",
        ];

        for package in &mcp_packages {
            info!(package = package, "Pre-caching MCP server npm package");

            // Use npx to pre-cache the package (same mechanism Codex uses at runtime)
            ...
            match output {
                Ok(output) if output.status.success() => {
                    info!(
                        package = package,
                        "MCP server package pre-cached successfully"
                    );
                }
                Ok(output) => {
                    ...
                    warn!(
                        package = package,
                        stderr = %stderr.trim(),
                        "MCP server package pre-cache had errors — Codex may experience MCP initialization delays"
                    );
                }
                Err(e) => {
                    warn!(
                        package = package,
                        error = %e,
                        "Failed to pre-cache MCP server package — npx not available or package not found. \
                         Codex may experience MCP initialization delays. Ensure npm/npx is installed."
                    );
                }
            }
        }
