...
"#;
            ...
            } else if contract.contains("status: AGREED") || contract.contains("status: \"AGREED\"")
            {
                ...
                format!(
                    ...
                    4. Commit and push your changes:\n\
                       - (git diff --name-only HEAD; git ls-files --others --exclude-standard) | grep -vE '(^|/)(\\.(codex|claude|agents|pair-shared))(/|$)|(^|/)\\.env($|/|\\.)|^worktrees/|^orchestration/|^\\.codex-home/' | xargs -r git add && git commit -m \"Segment N: <description>\"\n\
                       - git push -u origin HEAD (first push) or git push (subsequent)\n\
    ...
    fn rework_prompt(&self, shared: &Path) -> String {
        ...
        format!(
            ...
            - (git diff --name-only HEAD; git ls-files --others --exclude-standard) | grep -vE '(^|/)(\\.(codex|claude|agents|pair-shared))(/|$)|(^|/)\\.env($|/|\\.)|^worktrees/|^orchestration/|^\\.codex-home/' | xargs -r git add && git commit -F {shared_path}/COMMIT_MSG.md\n\
